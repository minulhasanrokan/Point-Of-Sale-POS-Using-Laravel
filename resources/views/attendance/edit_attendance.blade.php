    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <a style="width: 120px; float: right; margin-right: 10px; display: inline-block;" href="{{route('take.attendance')}}" type="button" class="btn btn-icon waves-effect waves-light btn-primary">
                    Take
                </a>
            ...
            <div class="card-body">
                <form method="post" action="{{route('update.attendance',$singleAttendance->id)}}">
                    @csrf
                <table  class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        <tr>
                            <td><img style="width: 40px; height: 40px;" src="{{asset('')}}employee/{{$employee->photo}}"></td>
                            <td>{{$employee->name}}</td>
                            ...
                            <td>
                                ...
                                <input type="hidden" name="user_id" id="userId_{{$employee->id}}" value="{{$employee->id}}">

                                <input type="hidden" name="att_year" value="{{date('Y')}}" class="form-control" id="att_year_{{$employee->id}}" placeholder="Expense Amount" required>
                                <input type="hidden" name="month" value="{{date('F')}}" class="form-control" id="month_{{$employee->id}}" placeholder="Expense Amount" required>
                                <input type="hidden" name="att_date" value="{{date('d/m/y')}}" class="form-control" id="att_date_{{$employee->id}}" placeholder="Expense Amount" required>
                            </td>
                            <td>
                                <input type="radio" @if($singleAttendance->attendance=="Present") Checked @endif name="attendance" id="attendance_{{$employee->id}}" value="Present"> Present
                                <input @if($singleAttendance->attendance=="Absence") Checked @endif type="radio" name="attendance" id="attendance_{{$employee->id}}" value="Absence"> Absence
                                
                            </td>
                        </tr>
                    </tbody>
                </table>
                <button style="float: right;" type="submit" class="btn btn-success">Update Attendance</button>
                </form>
            </div>
        </div>
    </div>
