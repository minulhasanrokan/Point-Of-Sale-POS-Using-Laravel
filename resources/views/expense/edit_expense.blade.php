    ...
    <div class="col-xl-6 ">
        <div class="card">
            <div class="card-header">
                <h3 style="display: inline-block;margin-top:10px ;" class="card-title">Edit Expense</h3>
                ...
                <a style="width: 120px; float: right; margin-right: 10px; display: inline-block;" href="{{route('add.expense')}}" type="button" class="btn btn-icon waves-effect waves-light btn-primary">
                    Add 
                </a>
            </div>
            @if ($errors->any())
            ...
            @endif
            <div class="card-body">
                <form method="post" id="add_expense" role="form" action="{{url('/update-expense',$singleExpense->id)}}">
                    @csrf
                    <div class="form-group">
                        <label>Expense Details</label>
                        <textarea name="details" id="details" class="form-control" required> {{$singleExpense->details}}</textarea>
                    </div>
                    <div class="form-group">
                        <label>Expense Amount</label>
                        <input type="number" name="amount" value="{{$singleExpense->amount}}" class="form-control" id="amount" placeholder="Expense Amount" required>
                    </div>
                    <div class="form-group">
                        <label style="display: none;">Expense Date</label>
                        <input type="hidden" name="date" value="{{$singleExpense->date}}" value="{{date('d/m/y')}}" class="form-control" id="date" placeholder="Expense Amount" required>
                    </div>
                    <div class="form-group">
                    ...
                    <button style="float: right" id="add_new_advanced_salary" type="submit" class="btn btn-purple waves-effect waves-light">Update</button>
                </form>
            </div>
            <!-- card-body -->
        </div>
